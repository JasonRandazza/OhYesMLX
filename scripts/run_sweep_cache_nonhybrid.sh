#!/bin/sh
# Candidate 3: the cold/warm KV split on a non-hybrid model (Llama-3.1-8B-oQ4).
# One cell per runtime (oq4), one run per (runtime, cache state), joined afterwards by
...
Q4_NONHYBRID="$H/models--brainworkup--Llama-3.1-8B-oQ4/snapshots/a041336af01fe59ffe17c762d4c970564dcabc53"
...
OUT=results/sweep-cache-nonhybrid
mkdir -p "$OUT"
...
for r in mlxlm omlx optiq vmlx osaurus; do
  for state in off on; do
    ...
    $PY -m ohyesmlx.cli run --study format --cells "oq4__$r=$Q4_NONHYBRID" --prompt-tokens 4096 \
