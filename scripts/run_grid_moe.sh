...
#
# Around the Osaurus column only, both ~/.osaurus/config files are copied byte-exact,
# modelIdleResidencyPolicy.seconds is pinned to 900 (the host's 30 unloads the model inside the
# 30 s cooldown, and the harness refuses to start Osaurus while the host differs from the
# recorded baseline), and both are restored byte-exact afterwards and on INT/TERM/HUP, checked
# with `cmp`. The other four columns see no settings write at all.
#
...
CONF_ORIG="$CONF.grid-moe-orig"
SERVER_ORIG="$SERVER.grid-moe-orig"
