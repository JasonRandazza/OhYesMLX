...
| Drift markers in a TTFT-ranked table are decode drift | Phase 6 | S | **FIXED 2026-09-17 in `render_sweep` only.** Entries carry the marker solely for decode-derived ranks (`DECODE_DERIVED_RANKS = {decode_tps}`); all other ranks print bare. Grids byte-identical, both TTFT sweeps marker-free, decode renders keep markers, 491 tests green. Two known follow-ups: (1) `ohyesmlx grid --rank ttft_p50_s` has the same latent mislabel (frozen by the fix order — needs its own order); (2) `CONCURRENCY_DRIFT_SENTENCE` now explains markers on tables that no longer show them when ranked non-decode. |
| Grid TTFT-ranked entries carry decode drift (same latent mislabel, `render_grid`) | Phase 6 | S | Found by the sweep-fix worker; frozen by its order. Needs its own order. |
| `CONCURRENCY_DRIFT_SENTENCE` now describes markers absent from non-decode tables | Phase 6 | S | Follow-up to the sweep fix; coordinator call whether to scope it per-rank. |
