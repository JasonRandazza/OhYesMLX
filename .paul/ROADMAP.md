...
- [x] 03-06: Native Multi-Token Prediction (MTP) in vMLX (`--enable-native-mtp` *[sic — no such flag; vMLX 1.6.59 uses `--native-mtp-depth`]*) — **Complete 2026-09-20** (`docs/research/2026-09-20-native-mtp-vmlx.md`). Evaluated 8 configurations across 3 semantic workloads in vMLX 1.6.59. Confirmed H1–H5: Native MTP at Fixed Depth 1 achieves a +31.0% decode speedup (78.7 -> 103.1 tok/s, confirmed 107.6 tok/s) on structured code and +27.9% (78.4 -> 100.3 tok/s) on philosophy at high acceptance (81.4%–85.3%); acceptance degrades monotonically with depth (D=1 [85%] > D=2 [71%] > D=3 [64%]); over-speculation beyond D=1 produces net throughput degradation (-15.5% at D=3 on architecture); memory overhead is negligible (+73 MB RAM); diagnosed upstream `Qwen3.6-35B-A3B-oQ4-mtp` artifact failure (0.0% acceptance, 41% decode collapse, token salad), contrasting with 100% clean non-MTP control.
...
- [x] Phase 4 (COMPLETE 2026-09-25, `0575ca4` + the MTP paper): Re-run the v3 script-based studies through the harness. Code 2026-09-24 (Decisions 122–124): `--kv-quant`, `--mtp-depth`, `--stream-experts`, `--workloads multiturn`. Sweeps 2026-09-25 (Decisions 125–126), four writeups `docs/research/2026-09-25-{kv-quant-sweep,expert-streaming-sweep,multiturn-runtime-axis,mtp-depth-sweep}.md`. The third 35B replicate was not run; it is carried in STATE Deferred Issues.
